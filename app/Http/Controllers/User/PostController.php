...
class PostController extends Controller
{
        ...
        try {
            ...
            if ($request->ajax()) {
                ...
                if ($input_search != null && isset($input_search)) {
                    ...
                    $data = Post::where('channel_id', $user['channel_id'])->where('title', 'LIKE', "%{$input_search}%")->orderBy('id', 'DESC')->get();
                } else {
                    $data = Post::where('channel_id', $user['channel_id'])->orderBy('id', 'DESC')->get();
                }
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        $delete = '<form onsubmit="return confirm(\'Are you sure !!! You want to Delete this Post ?\');" method="POST"  action="' . route('upost.destroy', [$row->id]) . '">
                            <input type="hidden" name="_token" value="' . csrf_token() . '">
                            <input type="hidden" name="_method" value="DELETE">
                            <button type="submit" class="edit-delete-btn" style="outline: none;" title="Delete"><i class="fa-solid fa-trash-can fa-xl"></i></button></form>';

                        $btn = '<div class="d-flex justify-content-around" title="Edit">';
                        $btn .= '<a href="' . route('upost.edit', [$row->id]) . '" class="edit-delete-btn">';
                        $btn .= '<i class="fa-solid fa-pen-to-square fa-xl"></i>';
                        $btn .= '</a>';
                        $btn .= $delete;
                        $btn .= '<a href="' . route('upostcontent.index', [$row->id]) . '" class="btn text-white p-1 font-weight-bold" title="Post Content" style="background: #4e45b8;"><i class="fa-regular fa-image fa-xl"></i></a>';
                        $btn .= '</a></div>';
                        return $btn;
                    })
            ...
            return view('user.post.index', $params);
        } catch (Exception $e) {
    ...
    public function create()
    {
        try {
            ...
            return view('user.post.add', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    ...
    public function edit(string $id)
    {
        try {
            ...
            if ($params['data'] != null) {
                ...
                return view('user.post.edit', $params);
            } else {
    ...
    public function destroy(string $id)
    {
        try {
            ...
            return redirect(route('upost.index'))->with('success', __('Label.data_delete_successfully'));
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function getpostcontent(Request $request)
    {
        try {
            ...
            return view('user.post.add_content', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
