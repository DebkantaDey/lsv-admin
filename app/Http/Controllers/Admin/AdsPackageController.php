...
class AdsPackageController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->ajax()) {
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        $delete = ' <form onsubmit="return confirm(\'Are you sure !!! You want to Delete this Package ?\');" method="POST"  action="' . route('adpackage.destroy', [$row->id]) . '">
                                <input type="hidden" name="_token" value="' . csrf_token() . '">
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" class="edit-delete-btn" style="outline: none;" title="Delete"><i class="fa-solid fa-trash-can fa-xl"></i></button></form>';

                        $btn = '<div class="d-flex justify-content-around" title="Edit">';
                        $btn .= '<a href="' . route('adpackage.edit', [$row->id]) . '" class="edit-delete-btn">';
            ...
            return view('admin.ads_package.index', $params);
        } catch (Exception $e) {
    ...
    public function create()
    {
        try {
            $params['data'] = [];
            return view('admin.ads_package.add', $params);
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function store(Request $request)
    {
        try {
            ...
            $package_data = Ads_Package::updateOrCreate(['id' => $requestData['id']], $requestData);
    ...
    {
        try {
            $params['data'] = Ads_Package::where('id', $id)->first();
            if ($params['data'] != null) {

                $this->common->imageNameToUrl(array($params['data']), 'image', $this->folder);
                return view('admin.ads_package.edit', $params);
            } else {
    ...
    public function destroy($id)
    {
        try {
            ...
            $data = Ads_Package::where('id', $id)->first();
            if (isset($data)) {
                $this->common->deleteImageToFolder($this->folder, $data['image']);
                $data->delete();
            }

            return redirect()->route('adpackage.index')->with('success', __('Label.data_delete_successfully'));
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
