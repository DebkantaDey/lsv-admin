...
class PodcastsController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            if ($request->ajax()) {
                ...
                $query = Content::where('content_type', 4)->with('channel');
                ...
                return DataTables()::of($data)
                    ...
                    ->addColumn('action', function ($row) {
                        $delete = ' <form onsubmit="return confirm(\'Are you sure !!! You want to Delete this Podcasts ?\');" method="POST"  action="' . route('podcasts.destroy', [$row->id]) . '">
                                <input type="hidden" name="_token" value="' . csrf_token() . '">
                                <input type="hidden" name="_method" value="DELETE">
                                <button type="submit" class="edit-delete-btn" style="outline: none;" title="Delete"><i class="fa-solid fa-trash-can fa-xl"></i></button></form>';

                        $btn = '<div class="d-flex justify-content-around" title="Edit">';
                        $btn .= '<a class="edit-delete-btn edit_podcasts" title="Edit" data-toggle="modal" href="#EditModel" data-id="' . $row->id . '" data-channel_id="' . $row->channel_id . '" data-title="' . $row->title . '" data-portrait_img="' . $row->portrait_img . '" data-landscape_img="' . $row->landscape_img . '" data-description="' . $row->description . '" data-category_id="' . $row->category_id . '" data-language_id="' . $row->language_id . '">';
    ...
    public function destroy($id)
    {
        try {
            ...
            return redirect()->route('podcasts.index')->with('success', __('Label.data_delete_successfully'));
        } catch (Exception $e) {
            return response()->json(array('status' => 400, 'errors' => $e->getMessage()));
        }
    }
    public function show($id)
    {
        try {
            ...
            $data = Content::where('id', $id)->with('channel')->first();
            if ($data->status == 0) {
                $data->status = 1;
