...
@section('page_title', 'Edit Coin Package')
...
@section('content')
	...
	<div class="right-content">
		...
		<div class="body-content">
			<!-- mobile title -->
			<h1 class="page-title-sm">Edit Coin Package</h1>

			<div class="border-bottom row mb-3">
				<div class="col-sm-10">
					<ol class="breadcrumb">
						<li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{__('Label.Dashboard')}}</a></li>
						<li class="breadcrumb-item"><a href="{{ route('adpackage.index') }}">Coin Package</a></li>
						<li class="breadcrumb-item active" aria-current="page">Edit Coin Package</li>
					</ol>
				</div>
				<div class="col-sm-2 d-flex align-items-center justify-content-end">
					<a href="{{ route('adpackage.index') }}" class="btn btn-default mw-120" style="margin-top:-14px">{{__('Label.Package List')}}</a>
				</div>
			</div>
			...
			<div class="card custom-border-card">
				<form id="package_update" enctype="multipart/form-data">
					...
					<div class="border-top pt-3 text-right">
						...
						<a href="{{route('adpackage.index')}}" class="btn btn-cancel mw-120 ml-2">{{__('Label.CANCEL')}}</a>
		...
		function update_package() {
            ...
            if(Check_Admin == 1){
                ...
                $.ajax({
                    ...
                    type: 'POST',
                    url: '{{route("adpackage.update", [$data->id])}}',
                    data: formData,
                    cache: false,
                    contentType: false,
                    processData: false,
                    success: function(resp) {
                        $("#dvloader").hide();
                        get_responce_message(resp, 'package_update', '{{ route("adpackage.index") }}');
                    },
