                                ...
                                <div id="{{$value->id}}" class="row listitemClass mb-2" style="background-color: #e9ecef;border: 1px solid black;cursor: s-resize;">
                                    <div class="col-md-10 mt-2">
                                        <h6>
                                            ...
                                            {{String_Cut($value->content->title, 100)}}
                                        </h6>
                                    </div>
...
@section('pagescript')
    <script>
        ...
        $("#content_type").change(function() {
            ...
            if(content_type == 1 || content_type == 2 || content_type == 4 || content_type == 6){
                ...
                $.ajax({
                    ...
                    type: 'post',
                    url: '{{ route("uplaylist.get.content") }}',
        ...
        function save_content(){
            ...
            $.ajax({
                type:'POST',
                url:'{{ route("uplaylist.content.save") }}',
                data:formData,
                cache:false,
                contentType: false,
                processData: false,
                success:function(resp){
                    $("#dvloader").hide();
                    get_responce_message(resp, 'content', '{{ route("uplaylist.content.index", $playlist_id) }}');
                },
                error: function(XMLHttpRequest, textStatus, errorThrown) {
                    $("#dvloader").hide();
                    toastr.error(errorThrown.msg,'failed');         
        ...
        function delete_content(id){
            ...
            $.ajax({
                ...
                enctype: 'multipart/form-data',
                type: 'POST',
                url: '{{ route("uplaylist.content.delete") }}',
        ...
        function getIdsOfList() {
            ...
            $.ajax({
                ...
                enctype: 'multipart/form-data',
                type: 'POST',
                url: '{{ route("uplaylist.content.sortable") }}',
                data: {ids:values},
                success: function(resp) {
                    // toastr.success(resp.success);
                },
